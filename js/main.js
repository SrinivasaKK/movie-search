...
function showMovieDetails(id, index){
 ...
 const movieDetails = sessionStorage.getItem(`${STATIC_TEXTS.SESSION_STORAGE_KEY}_${id}`);
 const content = document.getElementById(`content_${index}`);
  ...
  fetch(url).then(res => res.json())
    .then((response)=>{
    sessionStorage.setItem(`${STATIC_TEXTS.SESSION_STORAGE_KEY}_${id}`,JSON.stringify(response));
